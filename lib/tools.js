...
import { assertContainerRef } from './config.js';
...
/** 构建七个工具定义。 */
export function buildDockerTools(config, runner) {
    const cfg = config;
    ...
    const dockerManage = {
        ...
        async execute(rawArgs, exec) {
            ...
            if (!['start', 'stop', 'restart', 'rm'].includes(action))
                throw new Error('action 必须是 start / stop / restart / rm 之一（当前：' + action + '）。');
            const result = await runChecked(runner, manageArgs(cfg.dockerPath, action, container), timeout, 'docker ' + action, executionSignal(exec));
    ...
    const dockerHealth = {
        ...
        async execute(_rawArgs, exec) {
            ...
            try {
                ...
                if (executionSignal(exec)?.aborted === true)
                    throw error;
                ok = false;
